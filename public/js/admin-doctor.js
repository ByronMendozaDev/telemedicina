...
const registerUserDoctor = (email, password) =>
...
const onGetEspecialities = (callback) => db.collection("users").onSnapshot(callback);
const getEspeciality = (id) => db.collection('users').doc(id).get();
const deleteEspeciality = id => db.collection('users').doc(id).delete();
const updateEspeciality = (id, updatedEspeciality) => db.collection('users').doc(id).update(updatedEspeciality);
            ...
            if (doc.data().role === "doctor") {
                ...
                btnsEdit.forEach(btn => {
                    btn.addEventListener('click', async (e) => {
                        ...
                        register_especialityform['signup-full_name'].value = especialidad.fullname;
                        register_especialityform['signup-email'].value = especialidad.email;
                        register_especialityform['signup-user_name'].value = especialidad.username;
...
register_especialityform.addEventListener('submit', async (e) => {
    ...
    if (!editStatus) {
        await registerUserDoctor(email.value,password.value);
        console.log(password.value);
        await createEspeciality(email.value, especialidad.value, fullname.value, role, username.value);
